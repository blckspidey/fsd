...
import akhandaBharatMap from "@/assets/akhanda-bharat-map.jpg";

const Map = () => {
  const [activeFilters, setActiveFilters] = useState([]);
  const [activeRouteTemples, setActiveRouteTemples] = useState([]);
  ...
  const handleRouteToggle = (templeIds) => {
    ...
    <div className="min-h-screen pt-20">
      ...
      <section className="relative py-20 bg-gradient-to-b from-deep-bronze via-background to-card overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          ...
          <p
            className="text-xl text-parchment/80 font-body italic animate-divine-reveal"
            style={{ animationDelay: "0.2s" }}
          >
            Where divine energy flows through ancient land
          </p>
          <div
            className="mt-6 text-soft-gold/40 text-sm font-body animate-divine-reveal"
          ...
          <div className="relative bg-gradient-to-br from-sandstone/30 via-card to-deep-bronze/50 border-4 border-temple-gold/40 rounded-lg overflow-hidden shadow-temple">
            ...
            <div className="relative h-[75vh] overflow-hidden">
              <div className="absolute inset-0">
                <img
                  src={akhandaBharatMap}
                  alt="Ancient Bharat Map"
                  className="w-full h-full object-cover opacity-40"
                />
                <div className="absolute inset-0 bg-gradient-to-br from-sandstone/30 via-transparent to-deep-bronze/30 mix-blend-multiply" />
              </div>
              <div className="absolute inset-0 bg-gradient-to-br from-parchment/10 via-sandstone/20 to-deep-bronze/30">
                <div
                  className="absolute inset-0 border-8 border-temple-gold/20 rounded-lg"
                  style={{
                    borderImage:
                      "repeating-linear-gradient(45deg, hsl(var(--temple-gold) / 0.3), hsl(var(--temple-gold) / 0.3) 10px, transparent 10px, transparent 20px) 8",
                  }}
                />
                ...
                <svg className="absolute inset-0 w-full h-full opacity-30 pointer-events-none">
                  ...
                  <path
                    d="M 60% 20% Q 55% 35%, 50% 50% T 45% 80%"
                    stroke="url(#riverGradient)"
                    strokeWidth="3"
                    fill="none"
                    className="animate-[shimmer_4s_ease-in-out_infinite]"
                  />
                  <path
                    d="M 55% 25% Q 52% 40%, 48% 55%"
                    stroke="url(#riverGradient)"
                    strokeWidth="2"
                    fill="none"
                  ...
                  />
                  <path
                    d="M 55% 55% Q 60% 65%, 70% 75%"
                    stroke="url(#riverGradient)"
                    strokeWidth="2"
                    fill="none"
                    className="animate-[shimmer_6s_ease-in-out_infinite]"
                  />
                </svg>

                <div className="absolute top-0 left-0 right-0 h-1/4 bg-gradient-to-b from-parchment/20 to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 h-1/3 bg-gradient-to-t from-sandstone/20 to-transparent" />
